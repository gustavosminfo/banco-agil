"""
banco_agil/team.py
Equipe principal do Banco Ágil — compatível com Agno 2.6+.
...
Arquitetura:
  ...
  - SqliteDb: sessão sobrevive a recarregamentos da página Streamlit.
...
from banco_agil.config import MODEL_ID, SESSION_DB, MAX_AUTH_ATTEMPTS
...
def criar_equipe(session_id: str) -> Team:
    """
    Cria e retorna a equipe do Banco Ágil para uma sessão específica.

    Args:
        session_id: ID único da sessão (gerado pelo Streamlit).

    Returns:
        Team configurado e pronto para receber mensagens.
    """
    return Team(
        name="Banco Ágil",
        mode="coordinate",
        ...
        db=SqliteDb(db_file=str(SESSION_DB)),
