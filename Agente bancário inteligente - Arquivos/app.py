...
from banco_agil.team import criar_equipe, limpar_tags_da_resposta, extrair_info_auth
...
def _init_state() -> None:
    ...
    if "team" not in st.session_state:
        st.session_state.team = criar_equipe(st.session_state.session_id)
...
def _resetar_sessao() -> None:
    ...
    for key in ["session_id", "team", "messages",
...
def _renderizar_header() -> None:
    ...
    with col1:
        st.title("🏦 Banco Ágil")
        st.caption("Atendimento Virtual · Powered by Agno + Claude Sonnet 4.6")
    with col2:
        st.write("")
...
def _processar_mensagem(user_input: str) -> None:
    """Envia mensagem ao Team Agno e processa a resposta."""
    ...
    # 3. Montar contexto da sessão para injeção no prompt
    auth_ctx = _montar_contexto_auth()

    # 4. Chamar o Team Agno
    with st.chat_message("assistant", avatar="🏦"):
        with st.spinner("Analisando..."):
            try:
                run = st.session_state.team.run(
                    message=f"{auth_ctx}\n\nMensagem do cliente: {user_input}",
                    stream=False,
                )
                resposta_raw: str = run.content if hasattr(run, "content") else str(run)
    ...
    # 5. Extrair metadados e limpar tags
    _processar_tags_resposta(resposta_raw)
    resposta_limpa = limpar_tags_da_resposta(resposta_raw)

    # 6. Exibir e salvar resposta
    # Recria o chat_message para exibir após o spinner
    with st.chat_message("assistant", avatar="🏦"):
        st.markdown(resposta_limpa)
    st.session_state.messages.append(("assistant", resposta_limpa))

    # 7. Verificar encerramento
    if "encerrado" in resposta_raw.lower() or _detectar_encerramento(user_input):
        st.session_state.encerrado = True


def _montar_contexto_auth() -> str:
    """Monta string de contexto de autenticação para injetar no prompt do Team."""
    return (
        f"[CONTEXTO INTERNO — NÃO MOSTRAR AO CLIENTE]\n"
        f"autenticado: {st.session_state.autenticado}\n"
        f"cpf: {getattr(st.session_state, 'cpf_cliente', 'N/A')}\n"
        f"nome: {st.session_state.nome_cliente or 'N/A'}\n"
        f"tentativas_auth: {st.session_state.tentativas_auth}\n"
        f"max_tentativas: {MAX_AUTH_ATTEMPTS}\n"
    )
